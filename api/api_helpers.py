...
def generate_image_by_prompt_and_image(prompt, output_path, input_path, filename, save_previews=False):
  try:
    ws, server_address, client_id = open_websocket_connection()
    # upload_image(input_path, name, server_address, image_type="input").
    # Was a bug: client_id was passed positionally into image_type, sending an
    # invalid type to ComfyUI. The uploaded image's type must be "input".
    upload_image(input_path, filename, server_address, image_type="input")
    prompt_id = queue_prompt(prompt, client_id, server_address)['prompt_id']
    track_progress(prompt, ws, prompt_id)
    images = get_images(prompt_id, server_address, save_previews)
    save_image(images, output_path, save_previews)
...
def get_images(prompt_id, server_address, allow_preview = False):
  ...
  for node_id in history['outputs']:
      ...
      # Append one entry PER image. (Was a bug: file_name/type were set outside
      # the inner loop, so they leaked the last loop value and crashed/skipped
      # when a node produced zero or multiple images.)
      for image in node_output.get('images', []):
          is_preview = image['type'] == 'temp'
          if is_preview and not allow_preview:
              continue
